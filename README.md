...
|---|---|
| 状态S (262维) | 256（LSTM编码历史360步流量）+ 6（GPU数、计算利用率、显存利用率、延迟、冷却进度、副本数）|
| 动作A (Discrete 7) | 扩容+8/+16、缩容-8/-16、保持、切H100、切A100 |
...
|---|---|---|---|
| 平均奖励Reward | +0.0939 | -0.3854 | **+0.1185 (+26.2%)** |
| GPU利用率Util | 77.49% | 81.68% | 75.22% |
| SLA满足率SLA | 97.32% | 72.62% | 95.73% |
| 过载率Overload | 24.34% | 38.77% | **21.29%** |
| P95延迟(ms) | 1393.0 | 2500.2 | 1417.7 |
| 成本($/h) | 388.42 | 403.25 | 389.49 |
| 调整频率Adjust | 48.66% | 0.00% | **12.38%** |
| 调整惩罚Penalty | -0.4431 | +0.0000 | **-0.0697** |
